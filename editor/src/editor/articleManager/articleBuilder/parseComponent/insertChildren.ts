...
export function insertChildren(consistObj: ConsistObj, tempComps: TempCompTypes.TempComps) {
    ...
    if (Array.isArray(elemChildren) && elemChildren.length) {
    ...
    else if (!Array.isArray(elemChildren) && typeof elemChildren === 'object') {
        // Set additional attribute with text data component id
        // in order to get information about text component in element
        consistObj.htmlElem.children = [
            {
                tag: 'text-component',
                attrs: {
                    'data-em-d-comp-id': consistObj.dataComp.dCompId.toString(),
                    contentEditable: '',
                    suppressContentEditableWarning: ''
                },
                children: [{
                    text: elemChildren.text
                }]
            }
        ]
    }
    // Если в детях элементах ничего не указано, но там должен быть текст, то поставить текстозаполнитель
    else if (!elemChildren && consistObj.tempElem.elemTextInside) {
        consistObj.htmlElem.attrs['data-em-text-data-comp-id'] = consistObj.dataComp.dCompId.toString()
        ...
        if (!Array.isArray(consistObj.dElem.dCompElemChildren)) {
            consistObj.htmlElem.children = [
                {
                    tag: 'text-component',
                    attrs: {
                        // 'data-em-d-comp-id': consistObj.dataComp.dCompId.toString(),
                        'data-em-d-comp-id': consistObj.dElem.dCompElemChildren.dCompId.toString(),
                        contentEditable: '',
                        suppressContentEditableWarning: ''
                    },
                    children: [tElem]
