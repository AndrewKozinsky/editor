...
const initialState: SitesReducerType = {
    ...
    compFolderSection: {
        compFolderId: null, // id данных с папками (получается с сервера)
...
function setCompFolder(state: SitesReducerType, action: StoreSitesTypes.SetCompFolderAction): SitesReducerType {
    const updatedCompFolderSection = {
        ...state.compFolderSection,
        compFolder: action.payload.compFolder
    }
    if (action.payload.compFolderId) {
        updatedCompFolderSection.compFolderId = action.payload.compFolderId
    }

    return {
        ...state,
        compFolderSection: updatedCompFolderSection
    }
}
