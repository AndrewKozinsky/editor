 ...
 */
export function putRepeatedElems(htmlObj, dataComp) {
    if (!dataComp.dElems)
        return;
 ...
 */
function createGroupElemsMap(dataComp) {
    const groupElemsMap = {};
    for (let dataElem of dataComp.dElems) {
        ...
        groupElemsMap[groupName].push({ dCompElemId: dataElem.dCompElemId });
    }
 ...
 */
function setDuplicates(htmlParentArr, groupElemsMap, dCompId) {
    if (!htmlParentArr)
        return;
    for (let i = 0; i < htmlParentArr.length; i++) {
        const htmlChild = htmlParentArr[i];
        if ('text' in htmlChild)
            continue;
...
//# sourceMappingURL=putRepeatedElems.js.map