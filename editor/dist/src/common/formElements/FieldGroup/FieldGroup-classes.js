...
export default function makeClasses(vertical, gap) {
    return {
 ...
 */
export function getFieldGroupClasses(vertical, gap) {
    // Классы
    const classes = [CN];
    // Добавление класса дающего отступ между элементами внутри обёртки
    if (vertical)
        classes.push(CN + '--vertical');
    if (gap)
        classes.push(CN + '--gap' + gap);
    return makeCN(classes);
}
//# sourceMappingURL=FieldGroup-classes.js.map