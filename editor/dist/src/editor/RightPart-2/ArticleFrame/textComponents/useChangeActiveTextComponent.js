...
export default function useChangeActiveTextComponent() {
    ...
    useEffect(function () {
        ...
        const textCompId = (_a = historyItem === null || historyItem === void 0 ? void 0 : historyItem.selectedTextComp) === null || _a === void 0 ? void 0 : _a.dataCompId;
        if (!pressedKey.code || !textCompId || !$links)
            return;
...
function moveFocusStick(pressedKey, selection) {
    ...
    if (pressedKey.code === 'ArrowLeft') {
        ...
        if (pressedKey.shiftKey) {
            selection.setBaseAndExtent($text, anchorOffset, $text, focusOffset - 1);
        }
        else {
            selection.setPosition($text, focusOffset - 1);
        }
    }
    else if (pressedKey.code === 'ArrowRight') {
        // Запрет перемещения фокуса на несуществующую позицию
        if (focusOffset + 1 > $text.length)
            return;
        if (pressedKey.shiftKey) {
            selection.setBaseAndExtent($text, anchorOffset, $text, focusOffset + 1);
        }
 ...
 * @param {Object} selection — объект выделения на странице
 */
function selectAllText(selection) {
    // Текстовый узел, где есть выделение
    const $text = selection.anchorNode;
 ...
 */
function updateTextComp(selection, article, textCompId, pressedKey, $links) {
    const { anchorOffset, focusOffset } = selection;
    // 1. Получить выделенный текстовый компонент
    const dTextComp = articleManager.getComponent(article.article.dComps, textCompId);
    // 2. Сделать его копию
    ...
    const newArticle = makeImmutableObj(article, dTextComp, dTextCompCopy);
 ...
 * @param {Object} pressedKey — данные о нажатой клавише
 */
function getNewText(selection, currentText, pressedKey) {
 ...
 */
function setFocusToNewPosition($document, dTextComp, pressedKey, anchorOffset, focusOffset) {
...
//# sourceMappingURL=useChangeActiveTextComponent.js.map