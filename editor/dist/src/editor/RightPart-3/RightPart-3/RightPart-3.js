...
export default function RightPart3(props) {
    ...
    useEffect(function () {
        ...
        const parts = ['user', 'editor'].map((tabName) => {
            if (tabName === 'user') {
                return (React.createElement(HeaderPage, { headerText: userTabContentMessages.header[editorLanguage], display: tabName === settingsPanelTab, key: tabName },
                    React.createElement(SettingsUserTabContent, null)));
            }
            else if (tabName === 'editor') {
                return (React.createElement(HeaderPage, { headerText: editorTabContentMessages.header[editorLanguage], display: tabName === settingsPanelTab, key: tabName },
                    React.createElement(SettingsEditorTabContent, null)));
            }
        });
