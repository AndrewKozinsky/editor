...
export default function UserAccountForm() {
    ...
    const deleteAccount = useGetShowModal(React.createElement(ModalContent, null));
    // Хук возвращает функцию выводящую пользователя из учётной записи
    const logOut = useGetLogOut();
    return (React.createElement(Wrapper, { gap: 10 }, React.createElement(Button, { text: userAccountSectionMsg.deleteBtn, icon: 'btnSignTrash', onClick: deleteAccount }), React.createElement(Button, { text: userAccountSectionMsg.logOutBtn, icon: 'btnSignExit', onClick: logOut })));
}
