...
export default function Item(props) {
    ...
    return (React.createElement("div", { style: { paddingLeft: offset * 20 }, className: CN.root, "data-ft-item": itemData.id, draggable: 'true', onClick: onItemClickHandler, onMouseOver: markItemElem, onMouseOut: markItemElem, onDragStart: handleDragStart, onDrag: (e) => {
        ...
        } }, React.createElement("div", { className: CN.innerWrapper, "data-ft-inner": 'true' }, React.createElement(PlaceArrow, { itemData: itemData }), React.createElement(Triangle, { items: items, setItems: setItems, itemData: itemData, after: after }), React.createElement(Icon, { itemData: itemData }), itemData.name, React.createElement(Loading, { itemData: itemData }), React.createElement(RightButtons, { items: items, setItems: setItems, itemData: itemData, after: after }))));
}
...
function Triangle(props) {
    ...
    return (React.createElement("button", { className: CN.triangleBtn, onClick: toggleFolder, "data-ft-item-btn": 'true' }, React.createElement(SvgIcon, { type: 'filesTreeTriangle' })));
}
...
function Loading(props) {
    ...
    if (itemData.type !== 'file' || !itemData.loading)
        return null;
    return (React.createElement("div", { className: CN.loaderWrapper }, React.createElement(Loader, { className: CN.loader })));
}
/** Значок типа элемента. Если файл, то ничего не отрисовывается. */
function RightButtons(props) {
    const { items, setItems, itemData, after, } = props;
    const CN = makeClasses(itemData);
    const createFile = (React.createElement("button", { className: CN.rightBtn, "data-ft-item-btn": 'true', onClick: (e) => createNewFile(e, itemData, items, setItems, after) }, React.createElement(SvgIcon, { type: 'filesTreePlus' })));
    const createFolder = (React.createElement("button", { className: CN.rightBtn, "data-ft-item-btn": 'true', onClick: (e) => createNewFolder(e, itemData, items, setItems, after) }, React.createElement(SvgIcon, { type: 'filesTreeFolderPlus' })));
    const deleteItem = (React.createElement("button", { className: CN.rightBtn, "data-ft-item-btn": 'true', onClick: (e) => removeItem(e, items, setItems, itemData, after) }, React.createElement(SvgIcon, { type: 'filesTreeTrash' })));
    if (itemData.type === 'file' && itemData.loading) {
        return null;
    }
        ...
        return (React.createElement("div", { className: CN.rightPart }, createFile, createFolder, deleteItem));
    }
}
...
function PlaceArrow(props) {
    ...
    if (!placeMark || placeMark === 'inside')
        return null;
    return (React.createElement("div", { className: CN.placeArrow }, React.createElement(SvgIcon, { type: 'filesTreePlaceMark', extraClass: CN.placeArrowPointer }), React.createElement("div", { className: CN.placeArrowLine })));
}
