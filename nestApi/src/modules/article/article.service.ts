...
export class ArticleService {
    ...
    async createArticle(
        ...
        const site = await this.siteRepository.findOne({ id: newArticle.siteId })
        ...
        if (newArticle.siteTemplateId) {
            const siteTemplate = await this.siteTemplateRepository.findOne({ id: newArticle.siteTemplateId })
            // Throw an error if site is not exist
            if (!siteTemplate) {
